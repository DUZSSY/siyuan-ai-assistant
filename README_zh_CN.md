...
6. **AI来源责任**：本工具不对AI来源负责，使用者需确保所使用的AI服务提供商及使用方式符合当地法律法规。

7. **测试通道声明**：本工具提供的测试AI服务仅供测试使用，免费额度有限，不保证可用性。建议用户尽快配置自己的API Key。

8. **法律合规**：使用本工具请严格遵守当地法律法规。因使用本插件导致的任何法律问题，开发者不承担任何责任。
...
### v0.1.11
- ✨ 新增默认测试AI连接（基于 ai-proxy-pied.vercel.app），新用户开箱即用，无需配置
- ✨ 保留Ollama本地部署作为备选方案
- 📝 版本升级 (0.1.10 → 0.1.11)
